...
export default function Automation() {
    ...
    const getLogsMutation = useMutation({
        mutationKey: ['Logs'],
        mutationFn: async (id: string) => await AutomationModel.getAutomationLogs(id),
        onSuccess: (data) => {
            console.log(data)
        }
    })
            ...
            <Table>
                ...
                <TableBody>
                    {(automations.data as IAutomation[])?.map((automation, index) => (
                        <TableRow className="cursor-pointer" key={index} onClick={() => handleRowClick(automation.id)}>
                            <TableCell>{automation.id}</TableCell>
                            <TableCell>{format(new Date(automation.last_activity), 'dd. MM. yyyy HH:mm:ss')}</TableCell>
